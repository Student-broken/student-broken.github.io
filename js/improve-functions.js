...
function renderWidgets(etapeKey) {
    ...
    subjectsToRender.forEach(subject => {
        ...
        if (preferredView === 'line') renderLineGraph(chartCanvasId, overallSubject);
        else renderHistogram(chartCanvasId, overallSubject);
        ...
        widget.querySelector('.chart-view-toggle-btn').addEventListener('click', (e) => {
            ...
            mbsData.settings.chartViewPrefs[subject.code] = newView;
            ...
            if (newView === 'line') renderLineGraph(chartCanvasId, overallSubject);
            else renderHistogram(chartCanvasId, overallSubject);
        });
...
function renderGeneralAverageWidget(subjects, etapeKey) {
    ...
    if(history.length === 0 || history[history.length-1].toFixed(2) !== totalAverage.toFixed(2)) {
        history.push(totalAverage);
        if (history.length > 10) history.shift();
        mbsData.historique[historyKey] = history;
        localStorage.setItem('mbsData', JSON.stringify(mbsData));
    }
    ...
    if (preferredView === 'line') {
        renderGeneralAverageHistoryGraph(chartCanvasId, history);
        toggleBtn.innerHTML = '<i class="fa-solid fa-chart-column"></i>';
    } else {
        renderSubjectDistributionHistogram(chartCanvasId, subjects);
        toggleBtn.innerHTML = '<i class="fa-solid fa-chart-line"></i>';
    }
    
    toggleBtn.addEventListener('click', (e) => {
        e.stopPropagation();
        const currentView = mbsData.settings.chartViewPrefs[historyKey] || 'histogram';
        const newView = currentView === 'histogram' ? 'line' : 'histogram';
        mbsData.settings.chartViewPrefs[historyKey] = newView;
        localStorage.setItem('mbsData', JSON.stringify(mbsData));
        
        if (activeWidgetCharts[chartCanvasId]) activeWidgetCharts[chartCanvasId].destroy();
        if (newView === 'line') renderGeneralAverageHistoryGraph(chartCanvasId, history);
        else renderSubjectDistributionHistogram(chartCanvasId, subjects);
    });
}
...
function openExpandedView(subject) {
    ...
    summaryWidget.style.flexDirection = 'column';
    summaryWidget.innerHTML = `
        <div class="widget-top-section" style="flex: 1;">
            <div class="widget-info">
        ...
        <div class="histogram-container" style="flex: 1;"><canvas id="expanded-hist-chart"></canvas></div>
        <div class="histogram-container" style="flex: 1;"><canvas id="expanded-line-chart"></canvas></div>`;
    
    const detailsWidget = document.createElement('div');
    detailsWidget.className = 'subject-widget';
    detailsWidget.innerHTML = `
        <div style="display: flex; flex-direction: column; height: 100%;">
            <h3 class="widget-title" style="margin-bottom:20px; flex-shrink: 0;">Détails & Planificateur</h3>
            <div class="details-widget-body" style="flex-grow: 1; overflow-y: auto;">
            ...
            <a href="projection.html?subject=${encodeURIComponent(subject.name)}" class="btn-secondary" style="text-align: center; margin-top: 15px; flex-shrink: 0;">Plus d'information</a>
        </div>`;
    ...
    rankingWidget.dataset.rankingKey = rankingKey; // Store key for auto-refresh
...
function openExpandedViewForGeneral(subjects, etapeKey, average, history) {
    ...
    detailsWidget.innerHTML = `
        <div style="display: flex; flex-direction: column; height: 100%;">
            <h3 class="widget-title" style="margin-bottom:20px; flex-shrink: 0;">Détail par Matière</h3>
            <div class="details-widget-body" style="flex-grow: 1; overflow-y: auto;">
                <div class="subject-tag-container"></div>
                <div class="graph-container" style="height: 250px; margin-top: 20px; position: relative;"><canvas id="assignmentsChart"></canvas></div>
                <div class="calculator-container"><p>Le planificateur est disponible uniquement pour les matières individuelles.</p></div>
            </div>
            <a href="projection.html" class="btn-secondary" style="text-align: center; margin-top: 15px; flex-shrink: 0;">Plus d'information</a>
        </div>`;
...
function populateGeneralDetailsWidget(widget, subjects) {
    const subjectContainer = widget.querySelector('.subject-tag-container');
    subjectContainer.style.cssText = 'padding: 10px; text-align: center; max-height: 150px; overflow-y: auto;';
    subjects.forEach(subject => {
        const subjectTag = document.createElement('div');
        subjectTag.className = 'comp-widget';
        subjectTag.style.cssText = 'display: inline-flex; flex-direction: column; align-items: center; justify-content: center; min-width: auto; padding: 8px 12px; margin: 4px; cursor: default;';
        subjectTag.innerHTML = `<h4 style="margin: 0 0 4px 0; font-size: 0.9em;">${subject.name}</h4><div class="avg" style="font-size: 1.1em;">${subject.average.toFixed(1)}%</div>`;
...
async function fetchRankingData() {
    ...
    try {
        ...
        // --- GHOST LOADING FIX ---
        // If the expanded view is open, find the ranking widget and re-populate it.
        if (expandedViewOverlay.classList.contains('active')) {
            const rankingWidget = expandedViewGrid.querySelector('[data-ranking-key]');
            if (rankingWidget) {
                populateRankingWidget(rankingWidget, rankingWidget.dataset.rankingKey);
            }
        }
...
function populateRankingWidget(widget, rankingKey) {
    ...
    if (!currentUserData || !(rankingKey in currentUserData) || currentUserData[rankingKey] === null) {
        contentEl.innerHTML = `<p>Aucune donnée de classement pour cette catégorie.</p>`;
        return;
    }
    ...
    const leaderboardItemsHTML = levelData
        .filter(u => u[rankingKey] && !isNaN(parseFloat(u[rankingKey])))
        ...
        .map((user, index) => {
            ...
            return `<li class="leaderboard-item ${isCurrentUser ? 'is-user' : ''}">
                        ...
                        <span class="item-grade">${parseFloat(user[rankingKey]).toFixed(1)}%</span>
                    </li>`;
        }).join('');
    ...
    contentEl.innerHTML = `
        ...
        <div class="histogram-container" style="flex-grow: 1; margin-top:20px;"><canvas id="ranking-comparison-chart"></canvas></div>`;
...
// ... (Rest of the file remains the same, including openOrderEditor, calculateAveragesFromRawData, getRank, all chart rendering, and goal framework functions)
function openOrderEditor(subject) {
    ...
    list.addEventListener('dragend', e => { setTimeout(() => { if(draggedItem) { draggedItem.style.opacity = '1'; draggedItem = null; } }, 0); });
    ...
    const closeModal = () => { modal.remove(); renderWidgets(document.querySelector('.tab-btn.active').dataset.etape); };
    modal.addEventListener('click', closeModal);
    modal.querySelector('#save-order').addEventListener('click', () => { const newOrder = [...list.querySelectorAll('li')].map(li => li.dataset.id); mbsData.settings.assignmentOrder[subject.code] = newOrder; mbsData.settings.historyMode[subject.code] = 'assignment'; localStorage.setItem('mbsData', JSON.stringify(mbsData)); closeModal(); });
    modal.querySelector('#reset-mode-btn').addEventListener('click', () => { delete mbsData.settings.assignmentOrder[subject.code]; delete mbsData.settings.historyMode[subject.code]; localStorage.setItem('mbsData', JSON.stringify(mbsData)); closeModal(); });
...
function setupGoalFramework(subject, container) { container.innerHTML = `<h3>Planificateur d'Objectifs</h3><div class="goal-input"><label for="objective-input">Objectif :</label><input type="number" id="objective-input" min="0" max="100" value="">%</div><div id="calculator-content"></div>`; const objectiveInput = container.querySelector('#objective-input'); const calculatorContent = container.querySelector('#calculator-content'); const hasFutureWork = (subject.competencies || []).some(comp => (comp.assignments || []).some(a => getNumericGrade(a.result) === null && parseFloat(a.pond) > 0)); if (hasFutureWork) { setupIntraSubjectCalculator(subject, calculatorContent, objectiveInput); } else { calculatorContent.innerHTML = `<p>Tous les travaux pour cette matière ont été notés.</p>`; } }
function setupIntraSubjectCalculator(subject, container, goalInput) { container.innerHTML = `<p id="calc-info"></p><div id="goal-result" class="goal-result"></div>`; const goalResult = container.querySelector('#goal-result'); const calcInfo = container.querySelector('#calc-info'); const calculate = () => { let sumOfWeightedGrades = 0, sumOfCompletedWeights = 0, sumOfFutureWeights = 0; let totalSubjectWeight = 0; (subject.competencies || []).forEach(comp => { const compWeightMatch = comp.name.match(/\((\d+)%\)/); if (!compWeightMatch) return; const competencyWeight = parseFloat(compWeightMatch[1]); totalSubjectWeight += competencyWeight; (comp.assignments || []).forEach(assign => { const weight = parseFloat(assign.pond); if (isNaN(weight) || weight <= 0) return; const finalWeight = (weight / 100) * competencyWeight; const grade = getNumericGrade(assign.result); if (grade !== null) { sumOfWeightedGrades += grade * finalWeight; sumOfCompletedWeights += finalWeight; } else { sumOfFutureWeights += finalWeight; } }); }); if (sumOfFutureWeights.toFixed(4) < 0.0001) { calcInfo.textContent = 'Tous les travaux ont été notés.'; goalResult.style.display = 'none'; return; } const currentAverage = sumOfCompletedWeights > 0 ? (sumOfWeightedGrades / sumOfCompletedWeights) : 0; calcInfo.innerHTML = `Moyenne actuelle : <strong>${currentAverage.toFixed(2)}%</strong>`; const targetAvg = parseFloat(goalInput.value); if (isNaN(targetAvg) || targetAvg < 0 || targetAvg > 100) { goalResult.innerHTML = 'Veuillez entrer un objectif valide.'; goalResult.className = 'goal-result'; return; } const totalPointsNeeded = targetAvg * totalSubjectWeight; const pointsNeededFromFuture = totalPointsNeeded - sumOfWeightedGrades; const requiredAvgOnFuture = pointsNeededFromFuture / sumOfFutureWeights; let message, resultClass; if (requiredAvgOnFuture > 100.01) { message = `Il faudrait <strong>${requiredAvgOnFuture.toFixed(1)}%</strong> sur les travaux restants. Objectif impossible.`; resultClass = 'danger'; } else if (requiredAvgOnFuture < 0) { message = `Félicitations ! Objectif déjà atteint.`; resultClass = 'success'; } else { message = `Il vous faut une moyenne de <strong>${requiredAvgOnFuture.toFixed(1)}%</strong> sur les travaux restants.`; resultClass = 'warning'; } goalResult.innerHTML = message; goalResult.className = `goal-result ${resultClass}`; }; goalInput.addEventListener('input', calculate); calculate(); }
